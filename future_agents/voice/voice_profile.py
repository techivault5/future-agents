...
class VoiceProfile(BaseModel):
    ...
    # ── Acoustic Parameters ────────────────────────────────────────
    speaking_rate: float = Field(1.0, ge=0.5, le=2.5,
                                  description="Speech rate multiplier (1.0 = normal)")
    pitch_offset: float = Field(0.0, ge=-12.0, le=12.0,
                                 description="Semitone pitch offset from neutral")
    energy: float = Field(0.8, ge=0.0, le=1.0,
                           description="Energy/loudness normalised level")
    pause_factor: float = Field(1.0, ge=0.5, le=2.0,
                                 description="Inter-phrase pause duration multiplier")
    ...
    engine_config: dict[str, dict[str, Any]] = Field(
        ...
        description="Per-engine configuration overrides"
    )
