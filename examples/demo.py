...
async def main() -> None:
        ...
        "process.define",
        {
            ...
            "steps": [
                ...
                {"name": "Design", "description": "Create technical design document"},
                {
                    ...
                    "required_capabilities": ["Python Development"],
                },
                {
                    "name": "Code Review",
                    "description": "Get peer review",
                    "required_policies": ["code_review"],
                },
    ...
    print("  Gaps to Senior Engineer:")
    for gap in result["data"].get("gaps", []):
        print(
            f"    - {gap['skill_name']}: {gap['current']:.1f} → {gap['required']:.1f} (deficit: {gap['deficit']:.1f})"
        )
    ...
    print(
        f"  Python Development: level={cap['level']}, usage={cap['usage_count']}, "
    ...
    print(
        f"  Completed {result['data']['steps_completed']}/{result['data']['steps_completed']} steps"
    )
    ...
    for agent_id, info in health["agents"].items():
        print(
            f"    {info['type']:12s}  success_rate={info['success_rate']:.2f}  "
