...
SECRET_PATTERNS = [
    ...
    (
        "AWS_ACCESS_KEY",
        "AWS Access Key ID",
        r"(?i)(AKIA|AGPA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}",
        "critical",
    ),
    (
        "AWS_SECRET_KEY",
    ...
    ("GCP_API_KEY", "Google Cloud API Key", r"AIza[0-9A-Za-z\-_]{35}", "critical"),
    (
        "AZURE_CLIENT_SECRET",
        "Azure Client Secret",
        r"(?i)azure.{0,20}(secret|password|credential).{0,20}['\"][^'\"]{20,}['\"]",
        "critical",
    ),
    # Tokens - Auth & Communication
    ("GITHUB_TOKEN", "GitHub Token", r"gh[pousr]_[A-Za-z0-9_]{36,}", "critical"),
    ("GITLAB_TOKEN", "GitLab Token", r"glpat-[A-Za-z0-9\-]{20}", "critical"),
    ("SLACK_TOKEN", "Slack API Token", r"xox[baprs]-([0-9a-zA-Z]{10,48})", "critical"),
    (
        "SLACK_WEBHOOK",
        "Slack Webhook URL",
    ...
    ("TELEGRAM_BOT_TOKEN", "Telegram Bot Token", r"[0-9]{8,10}:[A-Za-z0-9_\-]{35}", "high"),
    # Database connection strings
    (
        "DB_CONNECTION_STRING",
        "Database Connection String with Password",
        r"(?i)(postgres(?:ql)?|mysql|mongodb|redis|mssql(?:\+\w+)?|sqlserver|oracle(?:\+\w+)?)://[^:@\s]+:[^@\s]+@[^\s/]+",
        "critical",
    ),  # noqa: E501
    (
        "SQLSERVER_ADO_CONNSTR",
        "SQL Server ADO.NET Connection String with Password",
        r"(?i)(Server|Data Source)=[^;]+;.*?Password\s*=[^;]{4,}",
        "critical",
    ),
    (
        "SQLSERVER_SA_PASS",
        "SQL Server SA / Trusted Connection with inline password",
        r"(?i)(User Id|UID)\s*=\s*\w+\s*;\s*Password\s*=\s*[^;\"']{4,}",
        "critical",
    ),
    (
        "DB_PASSWORD_INLINE",
        "Database Password in Config",
        r"(?i)(db_pass|database_password|db_password|PGPASSWORD|MSSQL_SA_PASSWORD|SA_PASSWORD)\s*[=:]\s*['\"]?[^'\"]{6,}['\"]?",
        "critical",
    ),  # noqa: E501
    # Generic credentials
    (
        ...
        r"(?i)(secret|password|passwd|pwd|token|api[_-]?key|auth[_-]?key)\s*[=:]\s*['\"][^'\"]{8,}['\"]",
        "high",
    ),  # noqa: E501
    (
        "PRIVATE_KEY_HEADER",
    ...
    ("CERTIFICATE", "Certificate/Key File", r"-----BEGIN CERTIFICATE-----", "medium"),
    # JWT
    (
        "JWT_TOKEN",
        "Hardcoded JWT Token",
    ...
    # OAuth / Auth services
    ("STRIPE_KEY", "Stripe API Key", r"sk_(live|test)_[A-Za-z0-9]{24,}", "critical"),
    ("SENDGRID_KEY", "SendGrid API Key", r"SG\.[A-Za-z0-9_\-]{22}\.[A-Za-z0-9_\-]{43}", "critical"),
    ("TWILIO_SID", "Twilio Account SID", r"AC[a-z0-9]{32}", "high"),
    (
        "TWILIO_TOKEN",
        "Twilio Auth Token",
        r"(?i)twilio.{0,20}(token|secret).{0,20}['\"][a-z0-9]{32}['\"]",
        "critical",
    ),
    ("OPENAI_KEY", "OpenAI API Key", r"sk-[A-Za-z0-9]{32,}", "critical"),
    ("ANTHROPIC_KEY", "Anthropic API Key", r"sk-ant-[A-Za-z0-9\-_]{40,}", "critical"),
    ("HUGGINGFACE_TOKEN", "HuggingFace Token", r"hf_[A-Za-z0-9]{30,}", "high"),
    # Cloud storage / CDN
    (
        "S3_BUCKET_URL_WITH_KEY",
        "S3 URL with embedded credentials",
        r"https?://[A-Za-z0-9]+:[A-Za-z0-9/+]{20,}@s3\.amazonaws\.com",
        "critical",
    ),
    # Containers / Registries
    ("DOCKER_AUTH", "Docker Registry Auth", r"\"auth\"\s*:\s*\"[A-Za-z0-9+/=]{20,}\"", "high"),
    # Sensitive env-var patterns
    (
        "ENV_SECRET",
        "Secret in .env file",
        r"^(?!#).*(SECRET|PASSWORD|TOKEN|KEY|PASS|CREDENTIAL)\s*=\s*.{6,}$",
        "high",
    ),
]
