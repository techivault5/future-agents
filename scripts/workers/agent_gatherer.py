        ...
        "skills": [
            {
                ...
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "target": {"type": "string", "description": "Target resource or identifier"},
...
def create_pr(gaps: list[dict], files: list[Path]) -> None:
    ...
    r = run([
        "gh", "pr", "create",
        "--title", f"[Agent Gatherer] Skeleton definitions for {domains}",
        "--body", "\n".join(body_lines),
        "--label", "automated,agent-gap",
        "--draft",
    ])
    if r.returncode == 0:
...
def create_gap_issue(gaps: list[dict]) -> None:
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    rows = "\n".join(
        f"| `{g['domain']}` | {g['importance']} | {g['description']} |"
        for g in gaps
    )
    body = "\n".join([
    ...
    r = run([
        "gh", "issue", "create",
        "--title", f"[Agent Coverage] {len(gaps)} uncovered domains ({today})",
        "--body", body,
        "--label", "automated,agent-gap",
    ])
    if r.returncode == 0:
